...
def frequency2distance(file_name, dir_path, func): 
    filename = os.path.join(dir_path, file_name)
    basename = os.path.splitext(filename)[0]
    outfile = os.path.join(dir_path, basename + ".txt")

    dtmp, ftmp = read_coordinates_of_unstable_point_from_dat(filename, 0, 1, 7)
    distmp = []
    ...
    with open(outfile, "w") as f:
        for i in range(len(dtmp)):
            ...
            f.write(r'%f, %f, %f' % (0.02, dtmp[i], disttemp))
            f.write('\n')
